...
namespace internals
{
template <typename Socket> class SocketGuard
{
public:
    template <typename... Args> explicit SocketGuard(Args&&... args) : socket(std::forward<Args>(args)...) {}
...
{
    [[maybe_unused]] DeviceConfiguration(std::string name, const std::string& deviceAddress, unsigned short port = 80)
        : name(std::move(name)),
...
private:
    ...
    template <typename... Arg> std::shared_ptr<R2000> static enableMakeShared(Arg&&... arg)
    {
...
public:
    ...
    bool asyncSendHttpCommand(const std::string& command, const ParametersMap& parameters,
                              AsyncCommandCallback callable, std::chrono::milliseconds timeout = 5000ms) noexcept(true);
