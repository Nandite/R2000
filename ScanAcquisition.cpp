...
void printMapRange(Iterator begin, Iterator end, ostream &stream) {
    auto count{0u};
    std::for_each(begin, end, [&stream, &count](const auto &node) {
...
int main(int argc, char **argv) {
    ...
    programOptionsDescriptions.add_options()("help,h", "Print out how to use the program")
            ("table", "Show the san resolution and frequency table for the supported devices.")
            ("udp", "Use the UDP transport layer protocol instead of the default TCP.")
            ("address,a", boost::program_options::value<std::string>()->default_value(""), "Address of the device.")
            ("target,t", boost::program_options::value<std::string>(),
             "If using the UDP protocol, address and port at which the scan datagrams must be sent. The address must be under the format <address:port>.")
            ("samples_per_scan,s", boost::program_options::value<std::string>()->default_value("7200"),
             "Number of points per scan.")
            ("scan_frequency,f", boost::program_options::value<std::string>()->default_value("35.0"),
             "Scan frequency")
    ...
    if (programOptions.count("watchdog") > 0) {
        ...
        if (!watchdogValue) {
            std::clog << "You must specify a valid watchdog timeout value (" << watchdogArgument
                      << ")." << std::endl;
            return EXIT_FAILURE;
        }
